...
public class MessageHandler {
    ...
    public  String handleMessage(Map<String,List<Map<String,Object>>> entities){
        if(EntityMap.entityExist(GREETINGS_MSG, entities))
         return  EntityMap.confidence(GREETINGS_MSG, entities) > 0.8 ? "Hello Sir!" : GENERIC_MSG;
        if(EntityMap.entityExist(LOCATION_MSG, entities))
            return  EntityMap.confidence(LOCATION_MSG, entities) > 0.8 ?  Weather.weather(EntityMap.value(LOCATION_MSG, entities), omwtoken) : GENERIC_MSG;
